...
- `deploy.py` describes creating a tiny `canary` schema + `canary.heartbeat`
  table via SQL; `health.py` describes a `SELECT count(*)`; `teardown.py`
  describes `DROP SCHEMA ... CASCADE`. All are P0 stubs (log intent, no live SQL).
